...
function mr_create_job {
    ...
    curl -s -F "job_name=$job_name" -F "model=$job_model" -F "zip_file=$job_file" http://$MR_SERVER:8080/jobs > $tmpfile
...
function mr_kill_job {
    ...
    local job_id=$1
    local tmpfile=$(mktemp -p $MR_TMP_DIR)
    curl -s http://$MR_SERVER:8080/jobs/$job_id/kill > $tmpfile
    cat $tmpfile |  mr_get_val_from_json message | grep OK > /dev/null
}
...
function mr_job_status {
    ...
    local job_id=$1
    local tmpfile=$(mktemp -p $MR_TMP_DIR)
    curl -s http://$MR_SERVER:8080/jobs/$job_id > $tmpfile
